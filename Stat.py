...
class Stat:
    ...
    def getDataAmount(self, data: int):
        size = "Bytes"
        if data:
            if data > 1023:
                data = data / 1024
        ...
        for i, row in enumerate(rows):
            self.dailyTable.insert(
                ...
                values=(
                    ...
                    row[2],
                    row[1],
                    self.getDataAmount(row[4]),
                    self.getDataAmount(row[3]),
                    self.getDataAmount(row[5]),
                ),
            )
    ...
    def fillSummaryTable(self):
        ...
        for date, value in dates.items():
            self.cursor.execute(
                f"SELECT SUM(received), SUM(sent), SUM(usage) FROM dailyStats WHERE date >= '{value}'"
            )
            row = self.cursor.fetchone()
            self.summaryTable.insert(
                parent="",
                index="end",
                iid=None,
                values=(
                    ...
                    "",
                    date,
                    self.getDataAmount(row[0]),
                    self.getDataAmount(row[1]),
                    self.getDataAmount(row[2]),
                ),
            )
